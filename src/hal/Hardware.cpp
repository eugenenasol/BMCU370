...
namespace Hardware {
    ...
    void InitUART(bool isKlipper) {
        ...
        if (isKlipper) {
            ...
            USART_InitStructure.USART_BaudRate = 115200;  // Reduced from 250000 to prevent UART overflow
    ...
    void UART_Send(const uint8_t *data, uint16_t length) {
        uart_tx_busy = true; // Mark as busy until ISR clears DE pin
        // Clear TC flag before transmission starts
        ...
        USART_ClearFlag(USART1, USART_FLAG_TC); // CRITICAL: Clear before enabling to prevent stale trigger
        USART_ITConfig(USART1, USART_IT_TC, ENABLE);
        ...
        if (DMA1_Channel4->CFGR & 0x01) {
             // Wait for data transfer to complete (DMA counter hitting 0)
             uint32_t timeout = 100000;
             while (DMA_GetCurrDataCounter(DMA1_Channel4) != 0 && timeout--) {
                 DelayUS(1);
             }
             // NOTE: We no longer wait for USART_FLAG_TC here because the ISR
             // clears it to handle the RS485 DE pin, which would cause a timeout stall.
        ...
        // CRITICAL: Enable TC interrupt so ISR can clear DE pin and uart_tx_busy flag
        // when the last byte is physically shifted out.
        USART_ClearFlag(USART1, USART_FLAG_TC); 
        USART_ITConfig(USART1, USART_IT_TC, ENABLE);
#endif
    }
    ...
    /* DEVELOPMENT STATE: FUNCTIONAL */
    /**
     * @brief Check if UART1 is currently busy transmitting.
     * @return true if transmission is in progress.
     */
    bool UART_IsBusy() {
        // 1. Check software flag (tracks until DE is disabled in ISR)
        if (uart_tx_busy) return true;

        // 2. Check DMA (Channel 4 is USART1 TX)
        if (DMA1_Channel4->CFGR & 0x01) { // Enabled
            if (DMA_GetCurrDataCounter(DMA1_Channel4) != 0) return true;
        }
