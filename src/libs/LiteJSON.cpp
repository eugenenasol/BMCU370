 ...
 *
 * This file implements all parsing and serialization logic for the LiteJSON
 * library.
 *
 * Key Implementation Details:
 * - Custom float parser (parseNumber) avoids stdlib atof crashes on FPU-less
 * MCUs
...
static void serializeValueFlat(const LiteValue &val, char *buf, size_t size,
                               size_t &offset) {
  auto write = [&](char c) {
    if (offset < size - 1)
      buf[offset++] = c;
  ...
  case ValueType::Null:
    ...
    break;
  case ValueType::Bool:
    writeStr(val.asBool() ? "true" : "false");
    break;
  case ValueType::Int: {
    char nbuf[16];
    snprintf(nbuf, sizeof(nbuf), "%d", val.asInt());
    writeStr(nbuf);
    break;
  }
...
bool LiteArray::add(const LiteValue &v) {
  ...
  case ValueType::Int:
    return add(v.asInt());
  case ValueType::Float:
    return add(v.asFloat());
  case ValueType::Bool:
    return add(v.asBool());
  case ValueType::String:
    return add(v.asString());
  case ValueType::Object:
...
void LiteArray::serializeElementFlat(int index, char *buf, size_t size,
                                     size_t &offset) const {
  if (index < 0 || index >= _size)
    return;
  const Element &e = _elements[index];
  ...
  case ValueType::Int: {
    char nbuf[16];
    snprintf(nbuf, sizeof(nbuf), "%d", e.val.i);
    writeStr(nbuf);
    break;
  }
  case ValueType::Float: {
  ...
  case ValueType::String:
    ...
    break;
  case ValueType::Object:
    serializeValueFlat(LiteValue((LiteObject *)getElementPtr(index)), buf, size,
                       offset);
    break;
  case ValueType::Array:
    serializeValueFlat(LiteValue((LiteArray *)getElementPtr(index)), buf, size,
                       offset);
    break;
  default:
    break;
...
int LiteObject::serialize(char *buf, size_t bufLen) const {
  size_t offset = 0;
  auto write = [&](char c) {
    if (offset < (size_t)bufLen - 1)
      buf[offset++] = c;
  };
  auto writeStr = [&](const char *s) {
    while (s && *s && offset < (size_t)bufLen - 1)
      buf[offset++] = *s++;
  };
...
const char *LiteDoc::parseArray(const char *p, LiteArray &arr, int depth) {
  if (!p || depth > MAX_NESTING) {
    _error = ParseError::NestingTooDeep;
    return nullptr;
  }
  if (*p != '[')
    return nullptr;
...
const char *LiteDoc::parseObject(const char *p, LiteObject &obj, int depth) {
  ...
  while (p && *p) {
    ...
    if (!p)
      break; // Check if parseString failed
...
LiteValue &LiteDoc::operator[](const char *key) { return _root[key]; }
...
const char *LiteDoc::errorString() const {
  ...
  case ParseError::TooManyKeys:
    return "Too many keys";
  case ParseError::BufferOverflow:
